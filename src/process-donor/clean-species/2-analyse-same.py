...
def get_round_number():
    round_file = r'data\donor-meta\species\round.csv'
    try:
...
def main():
    ...
    current_file = f'data\donor-meta\species\merged_species_{round_number}.xlsx'
    previous_file = f'data\donor-meta\species\merged_species_{round_number - 1}.xlsx'
    ...
    merged = pd.merge(df1, df2, on='species', suffixes=('_r1', '_r2'))
    ...
    for _, row in merged.iterrows():
        ...
        if json_rs_r1 == json_rs_r2:
            if json_rs_r1 == ['others']:
                same_others_rows.append({'species': species, 'json_rs': json_rs_r1})
            elif len(json_rs_r1) == 1:
                same_rows.append({'species': species, 'json_rs': json_rs_r1})
            else:
                different_rows.append({'species': species, 'json_rs_r1': json_rs_r1, 'json_rs_r2': json_rs_r2})
        else:
            different_rows.append({'species': species, 'json_rs_r1': json_rs_r1, 'json_rs_r2': json_rs_r2})

    output_same = f'data\donor-meta\species\same_{round_number}.xlsx'
    output_same_others = f'data\donor-meta\species\same-others_{round_number}.xlsx'
    output_different = f'data\donor-meta\species\species_{round_number + 1}.xlsx'
    output_different_csv = f'data\donor-meta\species\species_{round_number + 1}.csv'
