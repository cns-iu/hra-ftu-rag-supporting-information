...
def get_round_number():
    round_file = r'data\donor-meta\sex\round.csv'
    try:
...
def main():
    ...
    current_file = f'data\donor-meta\sex\merged_sex_{round_number}.xlsx'
    previous_file = f'data\donor-meta\sex\merged_sex_{round_number - 1}.xlsx'
    ...
    merged = pd.merge(df1, df2, on='sex', suffixes=('_r1', '_r2'))
    ...
    for _, row in merged.iterrows():
        ...
        if json_rs_r1 == json_rs_r2:
            if json_rs_r1 == ['others']:
                same_others_rows.append({'sex': sex, 'json_rs': json_rs_r1})
            elif len(json_rs_r1) == 1:
                same_rows.append({'sex': sex, 'json_rs': json_rs_r1})
            else:
                different_rows.append({'sex': sex, 'json_rs_r1': json_rs_r1, 'json_rs_r2': json_rs_r2})
        else:
            different_rows.append({'sex': sex, 'json_rs_r1': json_rs_r1, 'json_rs_r2': json_rs_r2})

    output_same = f'data\donor-meta\sex\same_{round_number}.xlsx'
    output_same_others = f'data\donor-meta\sex\same-others_{round_number}.xlsx'
    output_different = f'data\donor-meta\sex\sex_{round_number + 1}.xlsx'
    output_different_csv = f'data\donor-meta\sex\sex_{round_number + 1}.csv'
